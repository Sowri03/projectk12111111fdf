<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Software Asset Details</title>
</head>
<body style="font-family: Arial, sans-serif; background-color: #f4f6f8; padding: 20px;">

<div style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); max-width: 100000px; margin: 0 auto;">
    <h2 style="color: #2e3134; border-bottom: 2px solid #e5e7e9; padding-bottom: 10px;">Details about WiCOME_Bluetooth_stack Software asset</h2>

   

    <!-- Attribute: ID -->

    <div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

        <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">ID:</span>

        <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">3</span>

    </div>

 

    <!-- Attribute: Name -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Name:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">WiCOME_Bluetooth_stack</span>

</div>

 

<!-- Attribute: Description -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Description:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">Contains the core Bluetooth stack, protocol and profile implementation.  Bluetooth core stack is a third party  code received from OpenSynergy </span>

</div>

 

<!-- Attribute: Version -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Version:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">WB_mgu22_E_WiCome_23283A  Blue SDK 6.3 LE SDK 3.3 AVSDK 3.3 MTOBEX 4.3</span>

</div>

 

<!-- Attribute: Status -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Status:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">Active</span>

</div>

 

<!-- Attribute: Labels -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Labels:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">WB_mgu22_E_WiCome_23283A</span>

</div>

 

<!-- Attribute: Responsible -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Responsible:</span>

    <div style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">

        <strong>Name:</strong> Dey, Abhijit <br>

        <strong>Email:</strong> <a href="mailto:Abhijit.Dey@harman.com" style="color: #0077b5;">Abhijit.Dey@harman.com</a>

    </div>

</div>

 

<!-- Attribute: Architects -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Architects:</span>

    <div style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">

        <strong>Name:</strong> Kotanchery, Jibin<br>

        <strong>Email:</strong> <a href="mailto:Jibin.Kotanchery@harman.com" style="color: #0077b5;">Jibin.Kotanchery@harman.com</a>

    </div>

</div>

 

<!-- Continue with this structure for the rest of the fields ... -->




    <!-- Attribute: Last Review Date -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Last Review Date:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"></span>

</div>

 

<!-- Attribute: aSIL Level -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">aSIL Level:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">ASIL QM</span>

</div>

 

<!-- Attribute: Permissions -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Permissions:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">Restricted</span>

</div>

 

<!-- Attribute: Amount -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Amount:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">USD </span>

</div>

 

<!-- Attribute: Usage -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Usage:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="MGU22/21/18 NTG7/6 PSA Toyota MMT ManScania FPDM MSIL TATA GM Subaru" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: License -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">License:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"></span>

</div>

 

<!-- Attribute: Artifact Format -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Artifact Format:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">Executable</span>

</div>

 

<!-- Attribute: Layer -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Layer:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">Middleware</span>

</div>

 

<!-- Attribute: Domain -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Domain:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">Connectivity</span>

</div>

 

<!-- Continue with this structure for the rest of the fields ... -->

 

    <!-- Attribute: Environment -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Environment:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">Embedded</span>

</div>

 

<!-- Attribute: Requirements -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Requirements:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="https://alm.harman.com/rm3/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Falm.harman.com%2Frm3%2Fresources%2FMD_GVgDoaPZEem2zZVVqTlIhQ&componentURI=https%3A%2F%2Falm.harman.com%2Frm3%2Frm-projects%2F_tbH4cF0FEem_VbEj8XqWiA%2Fcomponents%2F_wArAUGAkEemrk_fCV6GB8Q&oslc.configuration=https%3A%2F%2Falm.harman.com%2Fgc%2Fconfiguration%2F1920  " style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Design Documents -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Design Documents:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="https://gerrit1.harman.com/plugins/gitiles/Connectivity/Development/WiCome/Common/+/refs/heads/master/doc/architecture/coc_connectivity_wicome_hdd.pdf " style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Subcomponents -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Subcomponents:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="Wicome_HostAgent,, Wicome_MonitorService, SASService, AliveService, BSSService, PhoneService, DmlService, PIMService, DSIService, SIMService, MediaService, BTMediaFSService,  AGService" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Release Notes -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Release Notes:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Resource Utilization -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Resource Utilization:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="CPU Utilization Connection - 2% Streaming / Contact Download / Call Scenario - 19%" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Interface -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Interface:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="https://gerrit1.harman.com/plugins/gitiles/Connectivity/Development/WiCome/Services/simservice/+/refs/heads/master/doc/GCFInterface_PSS_SimService.doc  https://gerrit1.harman.com/plugins/gitiles/Connectivity/Development/WiCome/Services/bssservice/+/refs/heads/master/doc/GIS_SCP_BSS.doc  https://gerrit1.harman.com/plugins/gitiles/Connectivity/Development/WiCome/Services/mediaservice/+/refs/heads/master/doc/MediaFSInterface_Generation4Plus.pdf  https://gerrit1.harman.com/plugins/gitiles/Connectivity/Development/WiCome/Services/mediaservice/+/refs/heads/master/doc/MediaFSInterface_Generation4.pdf  https://gerrit1.harman.com/plugins/gitiles/Connectivity/Development/WiCome/Services/phoneservice/+/refs/heads/master/doc/Phone%20GCF%20Interface%20Specification.doc https://gerrit1.harman.com/plugins/gitiles/Connectivity/Development/WiCome/Services/pimservice/+/refs/heads/master/doc/PIM%20Service%20Interface%20Specification.doc https://gerrit1.harman.com/plugins/gitiles/Connectivity/Development/WiCome/Services/audiogatewayservice/+/refs/heads/master/doc/AudioGateway%20GCF%20Interface%20Specification.doc https://gerrit1.harman.com/plugins/gitiles/Connectivity/Development/WiCome/Services/dmlservice/+/refs/heads/master/doc/GCFInterface_DMLSS_DmlMgr.doc"" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Runtime Dependencies -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Runtime Dependencies:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="Linux kernel, SQLite, zlib, Flexpp-Bisoncpp, libsqlite3, libexpat, ICU-lib, CxImage,   Curl and Libcurl" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Build Time Dependencies -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Build Time Dependencies:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="SDK2 provided by platform or customer, SOC packages, vendor packages (firmware, NVRAM/NVM, third party scripts, driver" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Continue with this structure for the rest of the fields ... -->




    <!-- Attribute: Repositories -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Repositories:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="ssh://gerrit1.harman.com:29418/Connectivity/Development/Integration/Gclient_Repos/iuk_mgu_wicome" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: FOSS -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">FOSS:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Supported OS -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Supported OS:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="Linux,QNX,Android" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Build Instructions -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Build Instructions:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href=" https://confluence.harman.com/confluence/display/COCCONN/CONNECTIVITY+BUILD+PROCESS#CONNECTIVITYBUILDPROCESS-HOWTOBUILD" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Origin -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Origin:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Activity -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">
    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Activity:</span>
    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto; word-wrap: break-word; overflow-wrap: break-word; overflow: hidden; text-overflow: ellipsis;">Received 12 gerrits in the month of June</span>
</div>

 

<!-- Attribute: Vulnerabilities -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Vulnerabilities:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="https://confluence.harman.com/confluence/display/COCCONN/BT+Vulnerabilities" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Code Language -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Code Language:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">C++,C</span>

</div>

 

<!-- Attribute: i18n -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">i18n:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">NA</span>

</div>

 

<!-- Attribute: Test Code -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Test Code:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="Test cases were developed using Test Complete. But the Test complete license has expired 2 years back. Dev test cases wherever available is captured in the next column" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Continue with this structure for any additional fields ... -->




    <!-- Attribute: Test Documentations -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Test Documentations:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;"><a href="" style="color: #0077b5;">Link</a></span>

</div>

 

<!-- Attribute: Forking -->

<div style="display: flex; justify-content: space-between; flex-wrap: wrap; margin-top: 20px;">

    <span style="color: #7a7d80; font-weight: bold; flex: 0 0 30%;">Forking:</span>

    <span style="line-height: 1.5; max-width: 65%; flex: 1 1 auto;">Not Allowed</span>

</div>









</div>

 

</body>

</html>
